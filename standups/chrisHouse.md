**Note-taker**: Chris House
**Daily Standup** - Monday
**Team Member**: Chris House

### What did you work on?
- Finalizing the frontend connection to the backend routes created for the playlists

### What are you going to work on?
- Creating the prisma models for users' comments and ratings

### Any near-future blockers and how can we support you?
- May have to reconstruct future task after reconsidering functionality (playlists specifically)

--- 

**Note-taker**:
**Daily Standup** - Tuesday
**Team Member**: 

### What did you work on?
- Testing frontend with new backend routes.

### What are you going to work on?
- Revisiting future tasks involving the prisma models

### Any near-future blockers and how can we support you?
- Waiting on my backend branch to be merged to 'dev'

--- 

**Note-taker**:
**Daily Standup** - Wednesday
**Team Member**: 

### What did you work on?


### What are you going to work on?


### Any near-future blockers and how can we support you?